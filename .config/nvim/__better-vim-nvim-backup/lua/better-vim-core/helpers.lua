...
M.get_config_item = function(keys)
  local bv = require "better-vim-core.better-vim-user-config"
  local config = bv.user_config
  local value = config
...
---@param theme_config table The theme plugin configuration
M.should_load_theme = function(theme_name, theme_config)
  local user_theme_name = M.get_config_item { "theme", "name" }
  return M.ternary(user_theme_name == theme_name, theme_config, {})
end
